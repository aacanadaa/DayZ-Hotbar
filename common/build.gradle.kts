/*
 * DayZ Hotbar - shared (loader-agnostic) module
...
dependencies {
    ...
    // The HUD uses no loader API of its own - it draws through vanilla
    // `GuiGraphics` and hooks the vanilla HUD - so Fabric Loader is the only
    // loader coordinate needed, and only for the few symbols the mixin config
    // expects to be resolvable. There is deliberately no Fabric API dependency:
    // this mod does not use it, and declaring one would tell users to install
...
// names Fabric actually runs with. The mixin config declares
// "refmap": "dayz-hotbar.refmap.json", so the generated file has to carry
